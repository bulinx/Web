...
<nav class="breadcrumb">
	...
	申请审核 <span class="c-gray en">&gt;</span> 转正申请
...
<div class="pd-20">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20" style="width: auto;">
        <span class="l">
            <a roles="3317-87" href="javascript:void(0);" onclick="stop();" class="btn btn-danger radius">
				<i class="icon-trash"></i> 批量退回
			</a>
            <a roles="2d29-87" href="javascript:void(0);" onclick="pass();" class="btn btn-primary radius">
				<i class="icon-plus"></i> 批量通过
			</a>
			<a roles="802e-87" href="ReportServer?reportlet=xgdj_normal.cpt" target="_blank" class="btn btn-default radius">
	...
	<table class="table table-border table-bordered table-hover table-bg table-sort" width="100%">
		...
		<c:forEach items="${lists}" var="list" varStatus="sta">
			<tr class="text-c">
				...
				<td>${list.prepTrain}</td>
				<td>${list.normalDate}</td>
				<td>${list.discuss}</td>
				<td>${list.normalApproveDate}</td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
