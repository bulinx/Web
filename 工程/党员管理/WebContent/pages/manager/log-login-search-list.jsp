...
<nav class="breadcrumb">
    ...
    日志管理 <span class="c-gray en">&gt;</span> 登录日志 <a
...
<div class="pd-20">
    <div class="text-c">
        <form action="log!search" method="post" id="sea">
            日期范围： <input type="text" placeholder="开始时间"
                         onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'%y-%M-%d %H:%m:%s'})"
                         id="datemin" name="start" class="input-text Wdate" style="width: 180px;" value="${start}">
            - <input type="text" placeholder="结束时间"
                     onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'%y-%M-%d %H:%m:%s'})"
                     id="datemax" name="end" class="input-text Wdate" style="width: 180px;" value="${end}">
            <input type="text" class="input-text" style="width: 150px"
                   placeholder="输入用户名" name="log.name" value="${log.name}" id="name">
            <input type="text" class="input-text" style="width: 150px"
                   placeholder="输入IP" name="log.ip" value="${log.ip}" id="ip">
            <input type="hidden" name="currentPage" id="currentPage">
            <button type="submit" class="btn btn-success">
                <i class="icon-search"></i> 搜日志
            </button>
    ...
    <table
        ...
        <c:forEach items="${lists}" var="list" varStatus="sta">
	        ...
	        <tr class="text-c">
	            ...
	            <td class="user-status"><span class="label label-success radius">${list.content}</span></td>
	        </tr>
	        </c:if>
	        <c:if test="${list.content=='登录失败'}">
	        <tr class="text-c">
	            <td>${sta.index+1}</td>
	            <td>${list.name}</td>
	            <td>${list.ip}</td>
	            ...
	            <td>${list.address}</td>
	            <td>${list.createTime}</td>
	            <td class="user-status"><span class="label radius">${list.content}</span></td>
	        </tr>
	        </c:if>
        </c:forEach>
        </tbody>
