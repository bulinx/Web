...
<div class="pd-20">
  <div class="text-c">
  <form action="dictionary!searchText" method="post">
      <span class="select-box" style="width:150px">
      <select class="select" id="dicId" name="dictionaryText.dicId">
      </select>
      </span>
      <input type="text" class="input-text" style="width:150px" placeholder="字典值" name="dictionaryText.name">
      <input type="text" class="input-text" style="width:150px" placeholder="字典编码值" name="dictionaryText.value">
  ...
  <div class="cl pd-5 bg-1 bk-gray mt-20">
    <span class="l">
      <a roles="88ba-62" href="javascript:void(0);" onclick="datadel()" class="btn btn-danger radius"><i class="icon-trash"></i> 批量删除</a>
    	<a href="javascript:void(0);" onclick="add('450','250','添加字典值','pages/manager/system-data-detail-add.jsp?dicId=${dicId}')" class="btn btn-primary radius">
    		<i class="icon-plus"></i> 添加字典值
    	</a>
    </span>
...
laypage({
    ...
    jump: function(e, first){ //触发分页后的回调
        if(!first){ //一定要加此判断，否则初始时会无限刷新
            location.href = 'dictionary!getText?currentPage='+e.curr+'&id='+'${dicId}';
        }
    }
});
