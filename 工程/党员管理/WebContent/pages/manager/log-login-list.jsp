...
<div class="pd-20">
    <div class="text-c">
        <form action="log!search" method="post">
            日期范围： <input type="text" placeholder="开始时间" id="datemin" name="start" 
            	onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'%y-%M-%d %H:%m:%s'})" 
            	class="input-text Wdate" style="width: 180px;">
            - <input type="text" placeholder="结束时间" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',maxDate:'%y-%M-%d %H:%m:%s'})"
                     id="datemax" name="end" class="input-text Wdate" style="width: 180px;">
            <input type="text" class="input-text" style="width: 150px"
                   placeholder="输入用户名" name="log.name" id="name">
            <input type="text" class="input-text" style="width: 150px"
                   placeholder="输入IP" name="log.ip" id="ip">
    ...
    laypage({
        ...
        jump: function(e, first){ //触发分页后的回调
            if(!first){ //一定要加此判断，否则初始时会无限刷新
            	window.location.href = "log?currentPage=" + e.curr;
            }
        }
    });
